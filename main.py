...
while not correct_input:
    try:
        m = int(input("Введіть кількість повторень: "))  # 3
        p = float(input("Введіть довірчу імовірність: "))  # 0.9
        correct_input = True
    except ValueError:
        pass
...
while not adequacy:
    while not homogeneity:
            ...
            for k in range(number_x):
                for x in range(N):
                    if k == 0:
                        ...
                        t_lst[x] += average_y[x] * in_matrix[x][k - 1] / N
        ...
        print(Fore.BLUE + "\tМатриця значень факторів")
        ...
        for i in range(len(matrix)):
            ...
            print("|")
        print("\n")
        print(Fore.BLUE + "\tРівняння регресії")
        print(Style.RESET_ALL)
        ...
        if m == 25:
            exit()
    print(Style.RESET_ALL)
    print(Fore.BLUE + "\tКритерій Стьюдента")
    print(Style.RESET_ALL)
    beta_1 = [b0, b1, b2, b3]
    ...
    if not fisher_test(significant_coefficients):
        ...
        for i in range(N):
            if i == 0:
                ...
                for j in range(7):
                    beta[i] += average_y[i] * in_matrix[i][j] / N

        print(Fore.BLUE + "\tРівняння регресії з ефектом взаємодії:")
        print(Style.RESET_ALL)
        print(
              "{:.3f} + {:.3f} * X1 + {:.3f} * X2 + {:.3f} * X3 + {:.3f} * Х1X2 + {:.3f} * Х1X3 + {:.3f} * Х2X3"
        ...
        if student_test(beta, 7):
            colorama.init()
            print(Fore.GREEN + "\t\tРівняння регресії адекватне оригіналу")
            adequacy = True
    else:
        colorama.init()
        print(Fore.RED + "\t\tРівняння регресії адекватне оригіналу")
        adequacy = True
